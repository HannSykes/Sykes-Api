...
const listkey = ["ohmyhannsyksz", "ilhanecha"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'ohmyhannsyksz') return res.sendFile(__path + '/views/eror.html')
...
router.get('/darkjokes', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'ohmyhannsyksz')  return res.json(invalidKeynya)
