...
const listkey = ["ilhansykes616", "ilhanecha"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'ilhansykes616') return res.sendFile(__path + '/views/eror.html')
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'ilhansykes616')  return res.sendFile(__path + '/views/eror.html')
...
router.get('/darkjokes', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'ilhansykes616')  return res.json(invalidKeynya)
...
router.get('/cekapikeyy', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'ilhansykes616') return res.json(loghandler.notparam)
    if (!text) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter text"})
