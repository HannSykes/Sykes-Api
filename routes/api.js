...
const listkey = ["hannsykes11", "ilhanecha"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'hannsykes11') return res.sendFile(__path + '/views/eror.html')
...
router.get('/simi', async (req, res, next) => {
    ...
    if (!q) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter"})
       ...
       fetch(encodeURI(`https://simsumi.herokuapp.com/api?text=${q}&lang=id`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/darkjokes', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'hannsykes11')  return res.json(invalidKeynya)
